...
const Sidebar: React.FC<SidebarProps> = ({ activeFeature, setActiveFeature, isOpen, setIsOpen, logout }) => {
  const navItems = [
  ...
  return (
    <aside className={`absolute md:relative z-20 w-64 bg-black/50 backdrop-blur-lg p-4 flex flex-col h-full transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
      ...
      <div className="mt-auto">
        <button
          onClick={logout}
          className="flex items-center w-full px-4 py-3 text-sm font-medium rounded-lg text-gray-400 hover:bg-red-500/20 hover:text-red-300 transition-colors duration-200"
        >
          <LogoutIcon className="w-5 h-5" />
          <span className="ml-4">Logout</span>
        </button>
        <div className="text-xs text-gray-500 text-center p-2 mt-2">
          Powered by Gemini
        </div>
      </div>
    </aside>
